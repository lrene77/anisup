...
<body>
	<div class="container">
		...
		<section>
			...
			<hr>
			<form method="get" action="${pageContext.request.contextPath}/admin/listboard/info/admin_info_list.do">
			...
			<hr>
			<form action="${pageContext.request.contextPath}/admin/listboard/info/admin_info_delete.do" method="post">
			<table class="table">
				<thead>
					<tr>
							...
							<c:forEach var="listboard" items="${listboardList}">
								<tr>
									...
									<td class="text-center">
										<c:url var="readUrl" value="/listboard/list_edit.do">
											<c:param name="listcate" value="${listboard.listcate}" />
											<c:param name="listno" value="${listboard.listno}" />
										</c:url> 
										<a href="${readUrl}" class="btn btn-primary" id="mbtn" style="color: white">수 정</a></td>
								</tr>
							</c:forEach>
						</c:when>
						<c:otherwise>
			...
			<%@ include file="/WEB-INF/inc/admin/info_bottom.jsp"%>
			</form>
		</section>
		<!-- 내용 끝 -->
	</div>
